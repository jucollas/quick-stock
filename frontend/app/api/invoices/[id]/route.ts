...
export async function GET(
  ...
  try {
    ...
    const upstream = await fetch(`${BILLING_BASE}/billing/facturas/${id}`, {
      method: "GET",
      ...
      const msg =
        (payload && typeof payload === "object" && ("detail" in payload || "message" in payload)
          ...
          : typeof payload === "string"
            ? payload
            : upstream.statusText) || "Error obteniendo factura";
      return jsonError(msg, upstream.status);
    }

    return NextResponse.json(payload);
  } catch (e: any) {
    return jsonError(e?.message || "Fallo obteniendo factura", 502);
  }
}
